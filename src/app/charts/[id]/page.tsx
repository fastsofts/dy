...
const ChartGenerate = ({ params }: { params: { id: string } }) => {
  ...
  const [updatedfilterdata , setUpdatedFilterData] = useState({values:[],labels:[]});
  const chartjsdefn = useRef<Chart | null>(null);;

  useEffect(() => {
    if (chartjsRef.current && filteredItems.length > 0 && filteredItems[0].chart === "chartjs") {
      const title = `${filteredItems[0].title}${filteredItems[0].filter.length > 0?' - ':''}${filteredItems[0].filter.length > 0?filteredItems[0].default:''}`;
      const labels:string[] = [];
      if (filteredItems[0].default && filteredItems[0].filter.length > 0){
        ...
        Object.keys(filteredItems[0].data[filteredItems[0].default]).forEach((key)=>{
  ...
  useEffect(() => {
    if (highchartRef.current && filteredItems.length > 0 && filteredItems[0].chart === "highchart") {
        ...
        if (filteredItems[0].default && filteredItems[0].filter.length > 0){
          Object.keys(filteredItems[0].data[filteredItems[0].default]).forEach((key)=>{
            if (filteredItems[0].split){
              ...
              values.push({"name":newVal,"y":filteredItems[0].data[filteredItems[0].default][key]})
            }else{
      ...
      console.log(labels)
      highchartRef.current = Highcharts.chart(highchartRef.current, {
        chart: {
          type: filteredItems[0].type, 
        },
        title: {
        ...
        series: [
          {
            name: title, // Name of the series
            data: values, // Data points for the bar chart
          },
        ],
        dataLabels: {
          enabled: true,
  ...
  return (
    <>
      {filteredItems && filteredItems.length > 0 && filteredItems[0].filter.length > 0 ? (
        <div className = "dropdown">
           <Select options={filteritems}  onChange={(e)=>changeData(e,filteredItems[0])} defaultValue={filteredItems[0].default} value={filteritems.value} />
        </div>   
      ):null} 
      {filteredItems && filteredItems.length > 0 && filteredItems[0].chart === "chartjs" ? (
        <div>
          <canvas
            width="500"
            height="550"
      ...
      ) : filteredItems && filteredItems.length > 0 && filteredItems[0].chart === "highchart" ? (
        <div
          id="higchart"
          className="chartstyle"
          ref={highchartRef}
